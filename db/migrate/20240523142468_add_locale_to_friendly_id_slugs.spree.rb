...
class AddLocaleToFriendlyIdSlugs < ActiveRecord::Migration[6.1]
  def change
    ...
    add_index :friendly_id_slugs, [:slug, :sluggable_type, :locale], length: { slug: 100, sluggable_type: 50, locale: 2 }
    remove_index :friendly_id_slugs, [:slug, :sluggable_type, :scope]
    add_index :friendly_id_slugs, [:slug, :sluggable_type, :scope, :locale], length: { slug: 60, sluggable_type: 50, scope: 60, locale: 2 }, unique: true, name: :index_friendly_id_slugs_unique
